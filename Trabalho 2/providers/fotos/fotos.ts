...
export class FotosProvider {
  ...
  inserirComentario(foto: any, comentario: string) {
    let fotoUpt = foto.payload.val();
    if (fotoUpt.comentario === undefined) {
      fotoUpt.comentario = [];  
    }
    fotoUpt.comentario.push(comentario);
    this.itemsRef.update(foto.payload.key, fotoUpt);
  }
